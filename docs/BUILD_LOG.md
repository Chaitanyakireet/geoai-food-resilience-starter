...
> **Historical record.** This is the original per-milestone engineering README,
> preserved verbatim from the build phase (Hours 0–44). It is accurate for what
> each milestone delivered *at the time*, but counts and "not yet implemented"
> notes below are frozen in time (for example, the interactive Food Network
> workspace described as pending at the end of this log was completed
> afterwards). For the current project overview see the root
> [README](../README.md) and [PROJECT_SUBMISSION.md](PROJECT_SUBMISSION.md).
